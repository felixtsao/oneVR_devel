...
`/player` - Basic VR 360 video/image player, specify video/image source and projection type in `viewer.js`, then launch `index.html` in a web browser (tested with Chrome and Firefox)
...
Located in `/player`, view demo by visiting [this link](/player/) on a desktop or smartphone web browser.
