...
0. Run SIFT (Scale Invariant Feature Transform) on cylindrical projections to detect feature points between all pairs of adjacent images. SIFT uses a Harris corner detector to filter the input images. Local areas with high corner responses from large differences in gradient eigenvalues are determined as unique objects in the image. Call the local maxima of these high response regions, feature points.
